...
int main(void)
{
     ...
     * OFB-8, so initialise the variable below to 8. 
     */
    int     numbits = 8;
    ...
    DES_ofb_encrypt(data, ciphertext, numbits, length, &schedule, (DES_cblock*)iv);
    ...
    /* First, copy the original IV data back to the IV array - as it 
     * was overwritten during the encryption process 
     */
    memcpy(iv, iv_data, DES_KEY_SZ);

    DES_ofb_encrypt(ciphertext, plaintext, numbits, length, &schedule, (DES_cblock*)iv);
