...
int main(void)
{
    ...
    int     length  = (strlen(data) + (RC5_32_BLOCK - 1)) & ~(RC5_32_BLOCK - 1);

    /* Input pointer to OpenSSL's RC5 CBC method must be a multiple of 8. */
    ...
    /* The output of OpenSSL's RC5 CBC method is always a multiple 8. */
    ...
    /* Encrypt the data */
    RC5_32_cbc_encrypt(data_to_encrypt, ciphertext, length, &key, iv, RC5_ENCRYPT);
    ...
    RC5_32_cbc_encrypt(ciphertext, plaintext, length, &key, iv, RC5_DECRYPT);
